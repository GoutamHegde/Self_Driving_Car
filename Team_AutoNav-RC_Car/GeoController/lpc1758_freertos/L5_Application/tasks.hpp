...
private:
	...
	void handleEchoAndBackspace(cmdChan_t *io, char c);bool saveDiskTlm(void);
};
...
public:
	...
	bool run(void *p) {
		run_GPS(); ///< This is a non-polling function if FreeRTOS is running.
		return true;
	}
};
...
public:
	periodicSchedulerTask(bool kHz = 0);bool init(void);bool regTlm(void);bool run(
			void *p);
